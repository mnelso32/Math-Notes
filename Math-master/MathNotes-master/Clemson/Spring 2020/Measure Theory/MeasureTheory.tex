...
We want to show that any finite premeasure defined on some algebra
$\mathcal{A}$ can be uniquely extended to a measure on $\sigma(\mathcal{A})$.
Let us state this as a theorem up front.

\begin{theorem}\label{theoremchkextension} (Caratheodory, Hahn, Kolmogorov)
Let $\mu$ be a finite premeasure defined on an algebra $\mathcal{A}$
of subsets of $X$. Then $\mu$ has a unique extension to a measure
on $\sigma(\mathcal{A})$. \end{theorem}

~~~The proof of this result is quite long and we will need to develop
it in steps.

\subsection{Defining a Pseudometric on $\mathcal{P}(X)$}

In $(C[a,b],\|\cdot\|)$, we define
\[
\mathrm{d}(f,g)=\|f-g\|
\]
for all $f,g\in C[a,b]$. If $A,B\in\sigma(\mathcal{A})$, then
\begin{align*}
\|1_{A}-1_{B}\| & =\int_{a}^{b}|1_{A}(x)-1_{B}(x)|\mathrm{d}x\\
 & =\int_{a}^{b}1_{A\Delta B}(x)\mathrm{d}x\\
 & =\mathrm{m}(A\Delta B).
\end{align*}
However we don't know what $\mathrm{m}(A\Delta B)$ is yet. This suggests
that the following pseudometric\footnote{See Appendix for important details on pseudometrics}
on $\sigma(\mathcal{A})$ might be useful. 

\begin{prop}\label{prop} Let $(X,\mathcal{A},\mu)$ be a premeasure
space. Define $\mathrm{d}_{\mu}\colon\mathcal{P}(X)\times\mathcal{P}(X)\to[0,\infty]$
by
\[
\mathrm{d}_{\mu}(A,B)=\mu^{*}(A\Delta B)
\]
for all $A,B\in\mathcal{P}(X)$. Then $\mathrm{d}_{\mu}$ is a pseudometric
on $\mathcal{P}(X)$. \end{prop}

\begin{proof} We first check reflexivity of $\mathrm{d}_{\mu}$.
Let $X\in\mathcal{P}(X)$, then have
\begin{align*}
...
Thus $(\mathcal{P}(X),\mathrm{d}_{\mu})$ is a pseudometric space.
The reason $\mathrm{d}_{\mu}$ is a pseudometric and not a metric
is because we not have identity of indiscernibles: we may have $\mu^{*}(A\Delta B)=0$
with $A\neq B$. All is not lost however as every pseudometric space
induces a metric space in a natural way. Let us briefly describe the
metric space induced by the pseudometric space $(\mathcal{P}(X),\mathrm{d}_{\mu})$.
More details can be found in the Appendix. We introduce an equivalence
relation $\sim$ on $\mathcal{P}(X)$ as follows: let $A,B\in\mathcal{P}(X)$.
Then
\[
...
complete. For this reason and many other reasons, we choose to work
in the pseudometric space $(\mathcal{P}(X),\mathrm{d}_{\mu})$ rather
than the metric space $([\mathcal{P}(X)],[\mathrm{d}_{\mu}])$. 
...
\cup(A,B)=A\cup B
\]
for all $(A,B)\in\mathcal{P}(X)$, is continuous on $[\mathcal{P}(X)]$.
\end{prop}
...
\mathrm{d}_{\mu}((A_{n}\cup B_{n}),(A\cup B)) & =\mu^{*}((A_{n}\cup B_{n})\Delta(A\cup B))\\
 ...
 & =\varepsilon.
\end{align*}
It follows that the union map is continuous on $[\mathcal{P}(X)]$.
\end{proof}
...
\begin{prop}\label{prop} The function $\mu^{*}\colon\mathcal{P}(X)\to[0,\infty]$
is Lipschitz continuous and is constant on equivalence classes. \end{prop}
...
|\mu^{*}(A)-\mu^{*}(B)| & \leq\max\{\mu^{*}(A\backslash B),\mu^{*}(B\backslash A)\}\\
 ...
 & =\mathrm{d}_{\mu}(A,B).
\end{align*}
It follows that $\mu^{*}$ is Lipschitz continuous. Let us check that
it is constant on equivalence classes. Let $A,A'\in\mathcal{P}(X)$
such that $A\sim A'$ (so $\mu^{*}(A\Delta A')=0$). Then
\begin{align*}
...
is complete. Thus $(\sigma(\mathcal{A}),\mathrm{d}_{\mu})$ is a completion
of $(\mathcal{A},\mathrm{d}_{\mu})$. Before doing this we introduce
the limsup and liminf of a sequence of sets.
...
\begin{prop}\label{prop} Let $\Sigma$ be any $\sigma$-algebra which
contains $\mathcal{A}$. Then $(\Sigma,\mathrm{d}_{\mu})$ is complete.
In particular, $(\sigma(\mathcal{A}),\mathrm{d}_{\mu})$ is complete.
\end{prop}

\begin{proof} Let $(A_{n})$ be a Cauchy sequence in $\Sigma$. To
show that $(A_{n})$ is convergent, it suffices to show that a subsequence
of $(A_{n})$ is convergent, by Lemma~(\ref{lemmaconvsubseq}). We
construct such a subsequence as follows. Since $(A_{n})$ is Cauchy,
...
\[
\mathrm{d}_{\mu}(A_{\pi(n)},A_{\pi(n+1)})<\frac{1}{2^{n}}.
\]

Now consider the subsequence $(A_{\pi(n)})$ of $(A_{n})$ and set
\[
A:=\limsup A_{\pi(n)}.
\]
We claim that $A_{\pi(n)}\to A$. To see this, we first observe that
for each $N\geq1$, we have
\begin{align*}
\bigcap_{\pi(n)\geq\pi(N)}A_{\pi(n)} & \subseteq\bigcup_{M\geq1}\bigcap_{\pi(m)\geq M}A_{\pi(m)}\\
 & =\liminf A_{\pi(m)}\\
 & \subseteq\limsup A_{\pi(m)}\\
 & =\bigcap_{M\geq1}\bigcup_{\pi(m)\geq M}A_{\pi(m)}\\
 & \subseteq\bigcup_{\pi(n)\geq\pi(N)}A_{\pi(n)}.
\end{align*}
Now let $\varepsilon>0$ and choose $N\geq1$ such that $2^{2-N}<\varepsilon$.
Then $\pi(n)\geq\pi(N)$ implies

\begin{align*}
\mathrm{d}_{\mu}(A,A_{\pi(n)}) & =\mu^{*}(A\Delta A_{\pi(n)})\\
 & =\mu^{*}(A\backslash A_{\pi(n)})+\mu^{*}(A_{\pi(n)}\backslash A)\\
 & \leq\mu^{*}\left(\bigcup_{\pi(m)\geq\pi(N)}A_{\pi(m)}\backslash\bigcap_{\pi(m)\geq\pi(N)}A_{\pi(m)}\right)+\mu^{*}\left(\bigcup_{\pi(m)\geq\pi(N)}A_{\pi(m)}\backslash\bigcap_{\pi(m)\geq\pi(N)}A_{\pi(m)}\right)\\
 & =2\mu^{*}\left(\bigcup_{\pi(m)\geq\pi(N)}A_{\pi(m)}\backslash\bigcap_{\pi(m)\geq\pi(N)}A_{\pi(m)}\right)\\
 & =2\mu^{*}\left(\bigcup_{\pi(m)\geq\pi(N)}(A_{\pi(m)}\Delta A_{\pi(m+1)})\right)\\
 & \leq2\sum_{\pi(m)\geq\pi(N)}\mu^{*}(A_{\pi(m)}\Delta A_{\pi(m+1)})\\
 & =2\sum_{\pi(m)\geq\pi(N)}\mathrm{d}_{\mu}(A_{\pi(m)}\Delta A_{\pi(m+1)})\\
 & <2\sum_{\pi(m)\geq\pi(N)}\frac{1}{2^{m}}\\
 & <2\sum_{m\geq N}\frac{1}{2^{m}}\\
 & =2^{2-N}\\
 & <\varepsilon.
\end{align*}
It follows that $(A_{\pi(n)})$ converges to $A$. Finally, note that
\begin{align*}
A & =\limsup A_{n}\\
 & =\bigcap_{N\geq1}\bigcup_{n\geq N}A_{n}\\
 & \in\Sigma.
\end{align*}
Thus every Cauchy sequence of sets in $\Sigma$ converges to a set
in $\Sigma$. Therefore $(\Sigma,\mathrm{d}_{\mu}|_{\Sigma})$ is
complete. \end{proof}

\subsubsection{$\mathcal{A}$ is dense in $(\sigma(\mathcal{A}),\mathrm{d}_{\mu})$}

Now we want to show that $\mathcal{A}$ is a dense subset of $\sigma(\mathcal{A})$.
In other words, we want to show that $\overline{\mathcal{A}}=\sigma(\mathcal{A})$,
...
\begin{prop}\label{prop} $\overline{\mathcal{A}}$ is a $\sigma$-algebra.
\end{prop}
...
\textbf{Step 1: }We first show that \textbf{$\mathcal{A}$ }is an
algebra. We have $\emptyset\in\mathcal{A}\subseteq\overline{\mathcal{A}}$.
...
it is countably subadditive. Choose sequences $(A_{n})$ and $(B_{n})$
...
Disjointify this sequence to $(B_{n})$: set $B_{1}=A_{1}$ and $B_{n}=A_{n}\backslash\bigcup_{i=1}^{n-1}A_{i}$
for all $n>1$. Then $(B_{n})$ is a sequence of pairwise disjoint
sets in $\overline{\mathcal{A}}$. Moreover
\[
\bigcup_{n=1}^{N}B_{n}=\bigcup_{n=1}^{N}A_{n}
\]
for all $N\in\mathbb{N}$. For each $n\in\mathbb{N}$, let $C_{n}=\bigcup_{i=1}^{n}B_{i}\in\overline{\mathcal{A}}$.
Then
\begin{align*}
\mathrm{d}_{\mu}\left(\bigcup_{n=1}^{\infty}A_{n},\bigcup_{n=1}^{N}A_{n}\right) & =\mathrm{d}_{\mu}\left(\bigcup_{n=1}^{\infty}B_{n},\bigcup_{n=1}^{N}B_{n}\right)\\
 & =\mu^{*}\left(\bigcup_{n=1}^{\infty}B_{n}\Delta\bigcup_{n=1}^{N}B_{n}\right)\\
...
\[
\bigcup_{n=1}^{\infty}A_{n}\in\overline{\mathcal{A}}.
\]
Thus $\overline{\mathcal{A}}$ is a $\sigma$-algebra. \end{proof}

\subsection{Uniqueness of Measure}

\subsubsection{Uniqueness of Extensions when Target Space is Hausdorff}

\begin{prop}\label{propuniquenessofextensions} Let $X$ be a topological
space and let $f\colon A\to Y$ be a continuous function from a dense
subspace $A$ of $X$ to a Hausdorff space $Y$. If there exists a
continuous extension of $f$ to all of $X$, then it must be unique.
In other words, suppose $\widetilde{f}_{1}\colon X\to Y$ and $\widetilde{f}_{2}\colon X\to Y$
...
\begin{proof} To prove uniqueness, assume for a contradiction that
$\widetilde{f}_{1}\colon X\to Y$ and $\widetilde{f}_{2}\colon X\to Y$
are two continuous extensions of $f$ such that $\widetilde{f}_{1}\neq\widetilde{f}_{2}$.
Choose $x\in X$ such that $\widetilde{f}_{1}(x)\neq\widetilde{f}_{2}(x)$.
...
\begin{prop}\label{prop} Let $\mu$ and $\nu$ be two finite measures
defined on $\sigma(\mathcal{A})$ which coincide on $\mathcal{A}$.
Then $\mu=\nu$. \end{prop}
...
We've just shown that any premeasure space $(X,\mathcal{A},\mu)$
can be extended (uniquely) to the measure space $(X,\sigma(\mathcal{A}),\mu^{*}|_{\sigma(\mathcal{A})})$.
It turns out that $(X,\mathcal{A},\mu)$ can be extended (uniquely)
to an even bigger measure space $(X,\mathcal{M},\mu^{*}|_{\mathcal{M}})$.
Let us define what this bigger measur space is. 
...
then every subset of $E$ is also a member of $\mathcal{M}$. This
is not necessarily true for $\sigma(\mathcal{A})$. In the special
case $\mathcal{A}=\mathcal{I}$ interval algebra, we call this $\sigma$-algebra
$\mathcal{M}$ the $\sigma$-algebra of \textbf{Lebesgue measurable
sets}. Recall that $\sigma(\mathcal{I})$ is called the $\sigma$-algebra
of \textbf{Borel measurable sets}. It is known that not every Lebesgue
measurable set is Borel measurable. The extension of the length measure
$\mathrm{m}$ from $\mathcal{I}$ to $\sigma(\mathcal{I})$ or $\mathcal{M}$
is called \emph{the} \textbf{Lebesgue measure}. By \emph{a} \textbf{Borel
...
~~~Let us introduce some more notation. Let $f\colon X\to\mathbb{R}$
and let $c\in\mathbb{R}$. We shall write
\begin{align*}
...
\{f\neq c\} & =\{x\in X\mid f(x)\neq c\}.
\end{align*}
This is common notation which is found throughout the literature. 

\subsubsection{Pointwise Convergence of Measurable Functions is Measurable}

\begin{prop}\label{proppointwiseconvmeasismeas} Let $(f_{n}\colon X\to\mathbb{R})$
be a sequence of functions which converges pointwise to a function
$f\colon X\to\mathbb{R}$. Then
\[
\{f\leq c\}=\bigcap_{k=1}^{\infty}\liminf_{n\to\infty}\{f_{n}<c+1/k\}.
\]
\end{prop}

\begin{proof} We have
\begin{align*}
x\in\bigcap_{k=1}^{\infty}\liminf_{n\to\infty}\{f_{n}<c+1/k\} & \iff x\in\liminf_{n\to\infty}\{f_{n}<c+1/k\}\text{ for all }k\\
 & \iff x\in\{f_{\pi_{k}(n)}<c+1/k\}\text{ for all }k\text{ for some subsequence }(\pi_{k}(n))_{n\in\mathbb{N}}\text{ of }(n)_{n\in\mathbb{N}}\\
...
\begin{cor}\label{corpointwiseconvmeasismeas} If $(f_{n}\colon X\to\mathbb{R})$
is a sequence of measurable functions which converges pointwise to
$f\colon X\to\mathbb{R}$, then $f$ is measurable. \end{cor}
...
\begin{proof} Corollary~(\ref{corpointwiseconvmeasismeas}) gives
us 2 implies 1 (since simple functions are measurable!), so we just
need to show 1 implies 2. Suppose $f$ is measurable. For each $n\in\mathbb{N}$
...
follows from (Proposition~(\ref{corpointwiseconvmeasismeas}). Therefore
...
\begin{defn}\label{defn} Let $f\colon X\to[-\infty,\infty]$ be a
measurable function. We define its \textbf{positive part $f^{+}\colon X\to[-\infty,\infty]$
}and its \textbf{negative part }$f^{-}\colon X\to[-\infty,\infty]$
by 
\[
f^{+}(x)=\max\{f(x),0\}\quad\text{and}\quad f^{-}(x)=-\min\{f(x),0\}
\]
for all $x\in X$. Note that both $f^{+}$ and $f^{-}$ are both nonnegative
measurable functions and $f=f^{+}-f^{-}$. We say $f$ is \textbf{integrable
}if 
\[
...
\begin{defn}\label{defn} A property is said to hold \textbf{almost
everywhere }with respect to $\mu$ (denoted $\mu$ a.e.) if it is
true everywhere except on a set of measure zero. \end{defn}

\subsection{$L^{1}(X,\mathcal{M},\mu)$}

~~~Note that if $f=0$ a.e., then 
\[
\int_{X}|f(x)|\mathrm{d}\mu(x)=0.
\]
Indeed, let $E=\{f\neq0\}=\{|f|\neq0\}$. Then $f=0$ a.e. implies
$\mu(E)=0$. Also $|f|=|f|\cdot1_{E}$. Let $0\leq\varphi\leq|f|$
be a simple function and express it in its canonical form:
\[
\varphi=\sum a_{i}1_{A_{i}}.
\]
Then for $a_{i}\neq0$ we have $A_{i}\subseteq E$. Indeed, if $\varphi(x)>0$,
then $|f(x)|>0$, which implies $x\in E$. Then
\begin{align*}
\int_{X}\varphi(x)\mathrm{d}\mu(x) & =\sum_{i=1}^{n}a_{i}\mu(A_{i})\\
 & \leq\sum_{i=1}^{n}a_{i}\mu(E)\\
 & =0.
\end{align*}
So 
\[
\int_{X}|f|\mathrm{d}\mu=\sup\{\int_{X}\varphi\mathrm{d}\mu\mid0\leq\varphi\leq|f|\}=0.
\]
We'll denote by $L^{1}(X,\mathcal{M},\mu)$ to be the set of all integrable
functions $f\colon X\to\mathbb{R}$ where any two such functions $f,g$
s.t. $f=g$ a.e. are identified. Equip this set with the following
norm:
\[
\|f\|_{1}:=\int_{X}|f(x)|\mathrm{d}\mu(x).
\]
This is a norm since
\begin{enumerate}
\item $\|f\|_{1}=0$ if and only if $f=0$ a.e. 
\item $\|cf\|_{1}=\int_{X}|cf(x)|\mathrm{d}\mu(x)=|c|\int_{X}|f(x)|\mathrm{d}\mu(x)=|c|\|f\|_{1}$
\item $|f(x)+g(x)|\leq|f(x)|+|g(x)|$ for all $x\in X$ implies
\begin{align*}
\|f+g\|_{1} & =\int_{X}|f+g|\mathrm{d}\mu\\
 & \leq\int_{X}|f|\mathrm{d}\mu+\int_{X}|g|\mathrm{d}\mu\\
 & =\|f\|_{1}+\|g\|_{1}.
\end{align*}
\end{enumerate}
Therefore $\|\cdot\|_{1}$ is indeed a norm, so $L^{1}(X,\mathcal{M},\mu)$
is a normed linear space.

\begin{theorem}\label{theorem} $L^{1}(X,\mathcal{M},\mu)$ is a Banach
space. \end{theorem}

To prove this we'll use the following general fact:

\begin{prop}\label{prop} A normed linear space $\mathcal{X}$ is
a Banach space if and only if every absolutely convergent series in
$\mathcal{X}$ is convergent. \end{prop}

~~~Recall that a series $\sum_{n=1}^{\infty}x_{n}$ in a normed
linear space is said to be convergent in the sequence of its partial
sums $s_{N}=\sum_{n=1}^{N}x_{n}$ is convergent. We say $\sum_{n=1}^{\infty}x_{n}$
is absolutely convergent if $\sum_{n=1}^{\infty}\|x_{n}\|$ is convergent. 

\begin{proof} Suppose $(f_{n})$ is a sequence in $L^{1}(X,\mathcal{M},\mu)$
such that $\sum_{n=1}^{\infty}\|f_{n}\|<\infty$. Denote by
\[
G_{N}=\sum_{n=1}^{N}|f_{n}|\quad\text{and}\quad G=\sum_{n=1}^{\infty}|f_{n}|.
\]
Then=
\begin{align*}
\|G_{n}\|_{1} & =\int_{X}|G_{N}|\mathrm{d}\mu\\
 & =\int_{X}\sum_{n=1}^{N}|f_{n}(x)|\mathrm{d}\mu(x)\\
 & =\sum_{n=1}^{N}\int_{X}|f_{n}|\mathrm{d}\mu\\
 & =\sum_{n=1}^{N}\|f_{n}\|_{1}\\
 & \leq\sum_{n=1}^{\infty}\|f_{n}\|_{1}\\
 & <\infty.
\end{align*}

Therefore by MCT it follows that
\begin{align*}
\int_{X}|G|\mathrm{d}\mu & =\int_{X}G\mathrm{d}\mu\\
 & =\lim_{N\to\infty}\int_{X}G_{N}\mathrm{d}\mu\\
 & =\lim_{N\to\infty}\|G_{N}\|_{1}\\
 & \leq\sum_{n=1}^{\infty}\|f_{n}\|_{1}\\
 & <\infty.
\end{align*}
This implies $G\in L^{1}(X,\mathcal{M},\mu)$. Let $F=\sum_{n=1}^{\infty}f_{n}$.
Why does $\sum_{n=1}^{\infty}f_{n}$ converge? We know that $G=\sum_{n=1}^{\infty}|f_{n}|$
is integrable. By a problem in HW6, this implies $G<\infty$ a.e.
Therefore $\sum_{n=1}^{\infty}f_{n}$ converges a.e., and hence this
allows us to define $F=\sum_{n=1}^{\infty}f_{n}$ a.e. Notice
\begin{align*}
|F| & =\left|\sum_{n=1}^{\infty}f_{n}\right|\\
 & \leq\sum_{n=1}^{\infty}|f_{n}|\\
 & =G
\end{align*}
a.e. We want to show that the sequence of partial sums $s_{N}=\sum_{n=1}^{N}f_{n}$
converges in $L^{1}$ to something. A natural guess for the limit
is $F$. Note that since $|F|\leq G$ and $G\in L^{1}(X,\mathcal{M},\mu)$,
we have $F\in L^{1}(X,\mathcal{M},\mu)$. So $F$ is a valid candidate.
So it remains to show $s_{N}\to F$ in $L^{1}$-sense, that is, $\|s_{N}-F\|_{1}\to0$.
We have
\begin{align*}
\|s_{N}-F\|_{1} & =\int_{X}|s_{N}(x)-F(x)|\mathrm{d}\mu(x)\\
 & =\int_{X}\left|\sum_{n=N+1}^{\infty}f_{n}(x)\right|\mathrm{d}\mu(x).
\end{align*}
We want to apply one of the convergence theorems. We'll use DCT. Set
$h_{N}=\left|\sum_{n=N+1}^{\infty}f_{n}\right|$. Then
\begin{align*}
|h_{N}| & =|s_{N}-F|\\
 & \leq|s_{N}|+|F|\\
 & \leq\sum_{n=1}^{N}|f_{n}|+|F|\\
 & \leq G+G\\
 & =2G,
\end{align*}
where $G$ is integrable. So we can apply DCT and obtain 
\begin{align*}
\lim_{N\to\infty}\|s_{N}-F\|_{1} & =\lim_{N\to\infty}\int_{X}h_{n}(x)\mathrm{d}\mu(x)\\
 & =\int_{X}\lim_{N\to\infty}h_{N}(x)\mathrm{d}\mu(x)\\
 & =\int_{X}0\mathrm{d}\mu(x)\\
 & =0.
\end{align*}
So we proved $s_{N}\to F$, that is, $\sum_{n=1}^{\infty}f_{n}$ converges
in $L^{1}$-sense. \end{proof}
...
\begin{prop}\label{prop} The set of all integrable functions is a
dense subspace of $L^{1}(X,\mathcal{M},\mu)$. \end{prop}
...
sequence of simple functions which converges pointwise to $f$. Set
$s_{n}=\varphi_{n}-\psi_{n}$. We claim that $\|s_{n}-f\|_{1}\to0$
as $n\to\infty$. In other words, we claim that
\[
\int_{X}|s_{n}-f|\mathrm{d}\mu\to0
\]
as $n\to\infty$. To this, we'll use DCT. Observe that
\begin{align*}
|s_{n}-f| & \leq|s_{n}|+|f|\\
 & =|\varphi_{n}+\psi_{n}|+|f|\\
 & =\varphi_{n}+\psi_{n}+|f|\\
...
Therefore
\begin{align*}
\lim_{n\to\infty}\int_{X}|s_{n}-f|\mathrm{d}\mu & =\int_{X}\lim_{n\to\infty}|s_{n}-f|\mathrm{d}\mu\\
...
Similar to $L^{1}$, we identify $f,g\in L^{p}(X,\mathcal{M},\mu)$
such that $f=g$ a.e. We equip this space with the $L^{p}$-norm
\[
\|f\|_{p}=\left(\int_{X}|f|^{p}\mathrm{d}\mu\right)^{\frac{1}{p}}.
\]
To show that this is a norm however will be slightly more difficult.
Indeed, the most difficult step is to show the triangle inequality
holds, so let's focus on this. The idea is to use the so called \textbf{Hölder's
inequality}.

\subsubsection{Hölder's Inequality}

\begin{prop}\label{prop} Let $1<p,q<\infty$ such that $1/p+1/q=1$.
If $f\in L^{p}(X,\mathcal{M},\mu)$ and $g\in L^{q}(X,\mathcal{M},\mu)$,
then $fg\in L^{1}(X,\mathcal{M},\mu)$ and 
\begin{equation}
...
In fact, this follows immediately from Young's inequality, which says
for all $a,b\geq0$ we have 
\[
ab\leq\frac{a^{p}}{p}+\frac{b^{q}}{q}.
\]
Indeed, we have
\begin{align*}
\int_{X}|fg|\mathrm{d}\mu & \leq\int_{X}\left(\frac{|f|^{p}}{p}+\frac{|g|^{q}}{q}\right)\mathrm{d}\mu\\
...
\end{align*}
After multiplying both sides by $\|f\|_{p}\|g\|_{q}$, we obtain Hölder's
inequality. \end{proof}

\subsubsection{Minkowski's Inequality}

\begin{prop}\label{prop} Let $1\leq p<\infty$ and $f,g\in L^{p}(X,\mathcal{M},\mu)$.
Then
\[
...
\begin{rem}\label{rem} This is just the triangle inequality for the
$L^{p}$-norm, but we call it Minkowski's inequality for historical
reasons. \end{rem}
...
We have
\begin{align*}
\int_{X}|f+g|^{p}\mathrm{d}\mu & \leq\int_{X}|f+g||f+g|^{p-1}\mathrm{d}\mu\\
 & \leq\int_{X}(|f|+|g|)|f+g|^{p-1}\mathrm{d}\mu\\
 & =\int_{X}|f||f+g|^{p-1}\mathrm{d}\mu+\int_{X}|g||f+g|^{p-1}\mathrm{d}\mu\\
 & \leq\|f\|_{p}\cdot\|(f+g)^{p-1}\|_{q}+\|g\|_{q}\cdot\|(f+g)^{p-1}\|_{q}\\
 & =(\|f\|_{p}+\|g\|_{q})\cdot\|(f+g)^{p-1}\|_{q}\\
 & =(\|f\|_{p}+\|g\|_{q})\left(\int_{X}|f+g|^{(p-1)q}\mathrm{d}\mu\right)^{\frac{1}{q}}\\
 & =(\|f\|_{p}+\|g\|_{q})\left(\int_{X}|f+g|^{p}\mathrm{d}\mu\right)^{\frac{1}{q}}.
\end{align*}
Thus
\begin{align*}
\left(\int_{X}|f+g|^{p}\mathrm{d}\mu\right)^{\frac{1}{p}} & =\left(\int_{X}|f+g|^{p}\mathrm{d}\mu\right)^{1-\frac{1}{q}}\\
 & \leq\|f\|_{p}+\|g\|_{p}.
\end{align*}
 \end{proof}
...
\begin{defn}\label{defnuniformconvergence} Let $(f_{n}\colon X\to\mathbb{R})$
be a sequence of functions and let $f\colon X\to\mathbb{R}$ be a
function. 
\begin{enumerate}
...
such that $f_{n}\to f$ uniformly on $E^{c}$. In other words, $(f_{n})$
converges to $f$ almost uniformly if for all $\varepsilon,\delta>0$
...
\begin{rem}\label{rem} If $(f_{n})$ converges to $f$ in the $L^{p}$-sense,
then we denote this by $f_{n}\xrightarrow{L^{p}}f$. \end{rem}

~~~Is it true that $f_{n}\xrightarrow{\mathrm{u}}f$ implies $f_{n}\xrightarrow{L^{1}}f$?
If $\mu(X)<\infty$, then the answer is yes! Indeed
\begin{align*}
\|f_{n}-f\|_{1} & =\int_{X}|f_{n}-f|\mathrm{d}\mu\\
 & \leq\sup|f_{n}-f|\mu(X)\\
 & \to0.
\end{align*}

Let's go over some more types of convergence implications:

\subsubsection{Convergence in $L^{1}$ Implies Convergence in Measure Zero}
...
Is it true that $f_{n}\xrightarrow{L^{1}}f$ implies $f_{n}\to f$
pointwise a.e.? The answer is no! Indeed, consider the sequence 
\begin{align*}
f_{1} & =1_{[0,1]}\\
f_{2} & =1_{[0,1/2]}\\
f_{3} & =1_{[1/2,1]}\\
...
f_{8} & =1_{[0,1/8]}\\
 & \vdots
\end{align*}
and so on. Then $f_{n}\xrightarrow{L^{1}}0$ but $f_{n}$ does not
converge pointwise to any function. Indeed, for any $x\in[0,1]$,
$f_{n}(x)=0$ for infinitely many $n$ and $f_{n}(x)=1$ for infinitely
many $n$. Similarly, $f_{n}\to f$ pointwise does \emph{not }imply
$f_{n}\xrightarrow{L^{1}}f$. 

\subsubsection{Convergence in Measure Zero Implies a Subsequence Converges Pointwise
Almost Everywhere}

\begin{prop}\label{prop} If $f_{n}\xrightarrow{\mathrm{m}}f$, then
there exists a subsequence $(f_{\pi(n)})$ of $(f_{n})$ such that
$f_{\pi(n)}\xrightarrow{\mathrm{pw}}f$ almost everywhere. \end{prop}

\begin{proof} Suppose $f_{n}\xrightarrow{\mathrm{m}}f$. Then for
each $m\in\mathbb{N}$, we can find a $\pi(m)\geq m$, such that $n\geq\pi(m)$
implies
\begin{equation}
\mu\left(\{|f_{n}(x)-f(x)|\geq1/2^{m}\}\right)<1/2^{m}.\label{eq:m0ispaeeq}
\end{equation}
...
$n=\pi(m)$! Indeed, for each $m\in\mathbb{N}$, let
\[
E_{m}:=\{x\in X\mid|f_{\pi(m)}(x)-f(x)|\geq1/2^{m}\}.
\]
So $\mu(E_{m})<1/2^{m}$, since this is a special case of (\ref{eq:m0ispaeeq})
where $n=\pi(m)$. Next, let
\[
E=\limsup E_{m}=\bigcap_{M\geq1}\bigcup_{m\geq M}E_{m}
\]
Observe that $\sum_{m=1}^{\infty}\mu(E_{m})<1$ implies $\mu(E)=0$
(by a previous HW problem). We claim that $(f_{n})$ converges pointwise
to $f$ on $E^{c}$. To see this, let $x\in E^{c}$. Then there exists
...
So $(f_{\pi(m)})$ converges to $f$ pointwise almost everywhere.
\end{proof}

\begin{cor}\label{cor} $f_{n}\xrightarrow{L^{1}}f$ implies there
exists a subsequence $(f_{n_{k}})$ such that $f_{n_{k}}\to f$ pointwise
a.e. \end{cor}

\subsubsection{On a Finite Measure Space, Pointwise a.e. Convergence Implies Almost
Uniform Convergence}

\begin{prop}\label{prop} If $\mu(X)<\infty$ and $f_{n}\xrightarrow{\mathrm{pw}}f$
almost everywhere, then $f_{n}\xrightarrow{\mathrm{au}}f$. \end{prop}

\begin{proof} Let 
\[
E_{n}(k):=\bigcup_{m\geq n}\{|f_{m}-f|\geq1/k\}
\]
for $n,k\in\mathbb{N}$. Then $E_{n}(k)\supseteq E_{n+1}(k)$ and
\[
\bigcap_{n=1}^{\infty}E_{n}(k)=\emptyset.
\]
Indeed, if $x\in\bigcap_{n=1}^{\infty}E_{n}(k)$, then there exists
a subsequence $(f_{\pi(n)})$ of $(f_{n})$ such that
\[
|f_{\pi(n)}(x)-f(x)|\geq1/k
\]
for all $n\in\mathbb{N}$, but this is a contradiction since $f_{n}(x)\xrightarrow{\mathrm{pw}}f(x)$.
Now, since $\mu(E_{1}(k))<\infty$, we have
...
Let $\delta>0$. Since $\mu(E_{n}(k))\to0$ as $n\to\infty$, there
exists $n_{k}\in\mathbb{N}$ such that
\[
\mu(E_{n_{k}}(k))<\frac{\delta}{2^{k}}.
\]
 Now let $E=\bigcup_{k=1}^{\infty}E_{n_{k}}(k)$. Then 
\begin{align*}
\mu(E) & \leq\sum_{k=1}^{\infty}\mu\left(E_{n_{k}}(k)\right)\\
 & <\sum_{k=1}^{\infty}\frac{\delta}{2^{k}}\\
 & =\delta.
\end{align*}
For $x\notin E$ we have $x\notin E_{n_{k}}(k)$ for all $k\in\mathbb{N}$
and hence
\[
|f_{n}(x)-f(x)|<\frac{1}{k}
\]
for all $n\geq n_{k}$ and $k\in\mathbb{N}$. Now let $\varepsilon>0$.
Choose $N=n_{k}$ where $1/k<\varepsilon$. Then for $n\geq N$, we
have
\[
|f_{n}(x)-f(x)|<\varepsilon
\]
for all $x\in E^{c}$. \end{proof}

\subsubsection{Convergences Diagram}

Let summarize the convergence implications in the following diagram
...
\int_{X\times Y}f\mathrm{d}(\mu\otimes\nu) & =\int_{X\times Y}f^{+}\mathrm{d}(\mu\otimes\nu)-\int_{X\times Y}f^{-}\mathrm{d}(\mu\otimes\nu).\\
 ...
 & =\int_{X}\left(\int_{Y}\left((f^{+})_{(-)}-(f^{-})_{(-)}\right)\mathrm{d}\nu\right)\mathrm{d}\mu\\
 & =\int_{X}\left(\int_{Y}\left((f_{(-)})^{+}-(f_{(-)})^{-}\right)\mathrm{d}\nu\right)\mathrm{d}\mu\\
 & =\int_{X}\left(\int_{Y}f_{(-)}\mathrm{d}\nu\right)\mathrm{d}\mu.
\end{align*}
...
because $f_{x}$ is $\mathcal{N}$-integrable almost everywhere. Indeed,
since
\[
...
we also have $1_{X}\in L^{2}(X,\mathcal{M},\mu)$. By Hölder's inequality,
we have
\begin{align*}
